...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (cleanEntities.length === 0) {
    return {
      title: `${actorName} - 影视作品与演职员资料 | iKanPP 爱看片片`,
      description: `查看${actorName}的演职员资料与影视作品档案。`,
      robots: { index: false, follow: false },
    };
  }

  const topWorks = cleanEntities.slice(0, 3).map(e => `《${e.title}》`).join('、');
  const title = `${actorName} - 影视作品与演职员资料 | iKanPP 爱看片片`;
  const description = `iKanPP 收录${actorName}参演的高口碑影视作品，包含${topWorks}等共${cleanEntities.length}部影视档案。查看在线剧情梗概与演职员详细资料。`;
