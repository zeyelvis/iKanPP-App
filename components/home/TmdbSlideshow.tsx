      ...
      <div className="absolute inset-x-0 bottom-0 z-20 w-full flex flex-col justify-end pb-2.5 sm:pb-3.5 pointer-events-none">
        <div className="fluid-container">
          <div className="iyf-hero-bar pointer-events-auto">
            ...
            {/* 1. 左栏：大气舒适尺寸，固定宽度与固定行高，大片片名与评分在上方，大播放按钮在下方贴底 */}
            <div className="w-[220px] sm:w-[240px] lg:w-[260px] xl:w-[280px] shrink-0 flex flex-col items-start justify-end">
              {/* 上方：相对放大、极具视觉冲击力的大片片名与评分，固定高度绝对零抖动 */}
              <div className="mb-2.5 sm:mb-3.5 w-full">
                <h2 className="text-xl sm:text-2xl lg:text-[30px] xl:text-[34px] font-black text-white tracking-tight drop-shadow-[0_4px_16px_rgba(0,0,0,0.95)] mb-1 truncate leading-tight h-8 sm:h-9 lg:h-10 flex items-center">
                  {active.title}
                </h2>
                <div className="text-white/90 text-xs sm:text-sm font-medium flex items-center gap-2 whitespace-nowrap drop-shadow-[0_2px_8px_rgba(0,0,0,0.9)] h-5.5">
            ...
            {/* 2. 中栏：爱壹帆同款高频热播追更速报列表（双排 6 列，恢复舒适字号 13px/14px，定宽杜绝挤压） */}
            {activeTrendingNav && activeTrendingNav.length > 0 && (
              <div
                className="hidden lg:grid grid-rows-2 grid-cols-6 items-center gap-x-2 xl:gap-x-3 gap-y-2 xl:gap-y-2.5 self-end shrink-1 max-w-[480px] xl:max-w-[530px] px-1 pb-0.5"
              ...
              >
                {activeTrendingNav.slice(0, 12).map((item: TrendingNavItem, idx: number) => {
                  ...
                  return (
                    ...
                    >
                      <span className={`text-[13px] xl:text-[14px] ${isCurrentActive ? 'font-bold' : 'font-medium'} tracking-tight whitespace-nowrap leading-snug truncate`}>
                        {item.title}
                      </span>
                      {item.updateBadge ? (
            ...
            {/* 3. 右栏：爱壹帆同款完整轮播大片缩略海报卡片列表（恢复饱满大尺寸，100% 完整展现不截断） */}
            <div className="hidden md:flex items-center gap-1 xl:gap-1.5 shrink-0 self-end p-1.5 rounded-xl bg-black/25 backdrop-blur-xs border border-white/10">
              {currentData.slice(0, 8).map((item, idx) => {
                ...
                const cardSizeClass = currentData.length > 7
                  ? 'w-11 h-15.5 sm:w-11.5 sm:h-16 lg:w-12 lg:h-17 xl:w-[50px] xl:h-[70px]'
                  : 'w-12 h-17 sm:w-12.5 sm:h-17.5 lg:w-13 lg:h-18.5 xl:w-[56px] xl:h-[78px]';
                return (
                  <button
                  ...
                  >
                    ...
                    <div className="absolute inset-x-0 bottom-0 bg-linear-to-t from-black/95 via-black/50 to-transparent pt-3 pb-0.5 px-0.5">
                      <p className="text-[8.5px] xl:text-[9.5px] text-white/90 truncate text-center font-medium leading-tight">
                        {item.title}
                      </p>
                    </div>
                  </button>
