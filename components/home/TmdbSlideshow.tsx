      ...
      <div className="absolute inset-x-0 bottom-0 z-20 w-full flex flex-col justify-end pb-3 sm:pb-4 lg:pb-5 pointer-events-none">
        <div className="fluid-container">
          <div className="iyf-hero-bar pointer-events-auto">
            ...
            <div className="w-[240px] sm:w-[270px] lg:w-[300px] xl:w-[320px] shrink-0 flex flex-col items-start justify-end">
              {/* 上方：相对放大、极具视觉冲击力的大片片名与评分，固定高度绝对零抖动 */}
              <div className="mb-3 sm:mb-4 w-full">
                <h2 className="text-2xl sm:text-3xl lg:text-4xl xl:text-[42px] font-black text-white tracking-tight drop-shadow-[0_4px_16px_rgba(0,0,0,0.95)] mb-1.5 sm:mb-2 truncate leading-tight h-9 sm:h-11 lg:h-13 flex items-center">
                  {active.title}
                </h2>
            ...
            {/* 2. 中栏：爱壹帆同款高频热播追更速报列表（双排 6 列，舒展大字 13px/15px，与播放按钮严格对齐） */}
            {activeTrendingNav && activeTrendingNav.length > 0 && (
              <div
                className="hidden lg:grid grid-rows-2 grid-cols-6 items-center gap-x-2.5 xl:gap-x-4 gap-y-2 xl:gap-y-2.5 self-end shrink-1 max-w-[500px] xl:max-w-[560px] px-1 pb-1"
              ...
              >
                {activeTrendingNav.slice(0, 12).map((item: TrendingNavItem, idx: number) => {
                  ...
                  return (
                    ...
                    >
                      <span className={`text-[13px] xl:text-[15px] ${isCurrentActive ? 'font-bold' : 'font-medium'} tracking-tight whitespace-nowrap leading-snug truncate`}>
                        {item.title}
                      </span>
                      {item.updateBadge ? (
            ...
            {/* 3. 右栏：爱壹帆同款完整轮播大片缩略海报卡片列表（饱满大尺寸海报，完整呈现绝不截断） */}
            <div className="hidden md:flex items-center gap-1.5 xl:gap-2 shrink-0 self-end p-1.5 xl:p-2 rounded-xl bg-black/25 backdrop-blur-xs border border-white/10">
              {currentData.slice(0, 8).map((item, idx) => {
                const isSelected = idx === activeIndex;
                const cardSizeClass = currentData.length > 7
                  ? 'w-11 h-15.5 sm:w-12 sm:h-17 lg:w-13 lg:h-18.5 xl:w-[56px] xl:h-[80px]'
                  : 'w-12 h-17 sm:w-13 sm:h-18.5 lg:w-14 lg:h-20 xl:w-16 xl:h-23';
                return (
                  <button
