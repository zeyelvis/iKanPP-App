...
export function WatchHistorySidebar({ isPremium = false }: { isPremium?: boolean }) {
  ...
  return (
    <>
      {/* Toggle Button (下移至下半区，彻底避免与巨幕翻页箭头冲突) */}
      <button
        onClick={() => setIsOpen(true)}
        className={`fixed right-6 bottom-24 z-40 hidden sm:flex bg-(--glass-bg) backdrop-blur-md saturate-120 border border-(--glass-border) rounded-2xl shadow-(--shadow-md) p-3 hover:scale-105 transition-all duration-300 cursor-pointer ${showToggle ? 'opacity-60 hover:opacity-100' : 'opacity-0 pointer-events-none'}`}
        aria-label="打开观看历史"
      >
        <Icons.History size={20} className="text-(--text-color)" />
      </button>
      ...
      <aside
        ...
        className={`fixed top-0 right-0 bottom-0 w-[88%] sm:w-[90%] max-w-105 z-2000 bg-[#0A0A0F]/95 backdrop-blur-3xl border-l border-white/10 p-6 flex flex-col shadow-2xl transition-transform duration-250 ease-out`}
      >
