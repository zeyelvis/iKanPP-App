...
/**
 * Records restored from server-side sync or corrupted local storage are not guaranteed
 * to be well formed — they may come from an older schema or a partial write.
 * Rendering one without `videoId` or `episodeIndex` throws while building the player URL,
 * which takes down the whole React tree, so anything lacking the required URL fields is dropped.
 */
