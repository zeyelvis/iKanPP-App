// 100% 真实对齐 huaren.live 原版官网数据资产
export interface HuarenRealCard {
    vodId: string;
    title: string;
    cover: string;
    badge: string;      // 右上角气泡（如“甜虐爱情”、“剧情”、“推理”、“喜剧”）
    badgeColor: string; // 气泡背景色 class
    episode: string;    // 右下角集数标签（如“16”、“第24集完结”、“第14集”）
    desc: string;       // 底部一句话剧情
}

export interface HuarenRealRank {
    rank: number;
    title: string;
    status: string;
}

export interface HuarenRealSection {
    id: string;
    title: string;
    rankTitle: string;
    type: 'movie' | 'tv';
    tags: string[];
    cards: HuarenRealCard[];
    rankings: HuarenRealRank[];
}

export const HUAREN_REAL_SECTIONS: HuarenRealSection[] = [
    {
        id: 'movies',
        title: '最新电影',
        rankTitle: '最新电影榜单',
        type: 'movie',
        tags: ['奇幻科幻', '战争犯罪', '悬疑恐怖惊悚', '爱情喜剧剧情', '动作冒险灾难', '动画电影'],
        cards: [
            {
                vodId: '200203',
                title: '消暑胜地反击战',
                cover: 'https://img.jisuimage.com/cover/5a591fff3a2ece097604ef705124246e.jpg',
                badge: '喜剧',
                badgeColor: 'bg-emerald-500 text-white',
                episode: '高清版',
                desc: '一个高度紧张的青少年发现自己被困在与她自由奔放的妈妈的老土男友度假...',
            },
            {
                vodId: '202326',
                title: '狼域',
                cover: 'https://hhmage.com/cover/c93324a2c83c1d57d4f2176b1873fd49.jpg',
                badge: '剧情',
                badgeColor: 'bg-blue-500 text-white',
                episode: '1080P',
                desc: '特种部队深入狼域绝境反击，展开殊死搏斗...',
            },
            {
                vodId: '196998',
                title: '痴心二人行',
                cover: 'https://img.jisuimage.com/cover/8b7c43d87834919d52136b64140c1456.jpg',
                badge: '爱情',
                badgeColor: 'bg-emerald-500 text-white',
                episode: '高清',
                desc: '在孟买，两个社交笨拙的干禧一代，在努力自我接纳的过程中找到了爱情...',
            },
            {
                vodId: '197109',
                title: '聊斋，魅首诡案',
                cover: 'https://img.jisuimage.com/cover/dfd426b67b513566a19a013ddd4b0da2.jpg',
                badge: '悬疑',
                badgeColor: 'bg-amber-500 text-black font-black',
                episode: 'HD',
                desc: '来自西域的舞姬阿离三年前惨死，背后的惊天诡案逐渐浮出水面...',
            },
            {
                vodId: '198465',
                title: '真实之物',
                cover: 'https://img.jisuimage.com/cover/fa15d3b256958c4114eb277ac66ccd21.jpg',
                badge: '剧情',
                badgeColor: 'bg-blue-500 text-white',
                episode: '1080P',
                desc: '在寻找心灵寄托的过程中，莱奥经历了一场关于人性与真理的心灵洗礼...',
            },
            {
                vodId: '200380',
                title: '对我来说你已经死了',
                cover: 'https://img.jisuimage.com/cover/198fa316e1f402c73db0cdc43efa641a.jpg',
                badge: '恐怖',
                badgeColor: 'bg-purple-600 text-white',
                episode: '高清版',
                desc: '这部电影聚焦一群即将离开家上大学的童年好友，在离别夜遭遇神秘诡事...',
            },
            {
                vodId: '200457',
                title: '痴人唱颂',
                cover: 'https://img.jisuimage.com/cover/df6121546e9c44610ea4ee9b7a11c93a.jpg',
                badge: '剧情',
                badgeColor: 'bg-emerald-500 text-white',
                episode: 'HD',
                desc: '在一个与世隔绝的村庄里，年轻的唱颂者揭开被尘封百年的秘密...',
            },
            {
                vodId: '202325',
                title: '蝙蝠侠：骑士陨落第一部',
                cover: 'https://hhmage.com/cover/488c051af3c09185cf58987d56551094.jpg',
                badge: '动作',
                badgeColor: 'bg-blue-500 text-white',
                episode: '1080P',
                desc: '哥谭黑夜再临危机，贝恩策划惊天越狱，蝙蝠侠迎战最强劲敌...',
            },
            {
                vodId: '198460',
                title: '哆啦A梦，新大雄的海底鬼岩城',
                cover: 'https://hhmage.com/cover/4497c7e72bb59c84ad147d9639dfbd05.jpg',
                badge: '豆瓣热榜',
                badgeColor: 'bg-amber-500 text-black font-black',
                episode: '完整版',
                desc: '哆啦A梦 2026全新作电影降临，大雄与伙伴们开启深海海底奇幻冒险...',
            },
            {
                vodId: '202334',
                title: '罪火焦点',
                cover: 'https://hhmage.com/cover/8e4f08f754338c19a1b86bcd8ccafce2.jpg',
                badge: '犯罪',
                badgeColor: 'bg-amber-500 text-black font-black',
                episode: 'HD',
                desc: '故事设定于AI统治的近未来，一名老派警探在赛博罪恶都市中孤身追凶...',
            },
        ],
        rankings: [
            { rank: 1, title: '给阿嬷的情书', status: '已完结' },
            { rank: 2, title: '夜王', status: '已完结' },
            { rank: 3, title: '蜘蛛侠：崭新之日', status: '高清版' },
            { rank: 4, title: '痴迷', status: '第1集' },
            { rank: 5, title: '哪吒之魔童闹海', status: '第1集' },
            { rank: 6, title: '疯狂动物城2', status: '已完结' },
            { rank: 7, title: '飞驰人生3', status: '已完结' },
            { rank: 8, title: '一路向西', status: '已完结' },
            { rank: 9, title: '群体', status: '已完结' },
            { rank: 10, title: '色戒', status: '已完结' },
        ],
    },
    {
        id: 'tvs',
        title: '最新剧集',
        rankTitle: '最新剧集榜单',
        type: 'tv',
        tags: ['大陆剧', '港台剧', '日韩剧', '欧美剧', '其他剧'],
        cards: [
            {
                vodId: '200158',
                title: '春花宴',
                cover: 'https://static.huarenlivewebsite.top/piccdn/2/14/chun_hua_yan/200158_image.png',
                badge: '甜虐爱情',
                badgeColor: 'bg-emerald-500 text-white',
                episode: '16',
                desc: '暗厂死士眉林身负血仇，以命入局，周旋于多位男子之间...',
            },
            {
                vodId: '201756',
                title: '欲望的陷阱(2026)',
                cover: 'https://static.huarenlivewebsite.top/piccdn/2/16/yu_wang_de_xian_jing/201756_image.png',
                badge: '剧情',
                badgeColor: 'bg-emerald-500 text-white',
                episode: '13',
                desc: '本剧讲述被冤枉为杀人而失去生命的一名女性面对巨大的欲望，重新找回自己人生的复仇剧...',
            },
            {
                vodId: '202351',
                title: '月蚀迷城',
                cover: 'https://hhmage.com/cover/5d3c0362c7c9e17c3b023c237ed33bfc.jpg',
                badge: '剧情',
                badgeColor: 'bg-rose-500 text-white',
                episode: '第24集完结',
                desc: '被陷害成为黑帮卧底的失忆警长沈天白，与落魄黑帮少爷陆野，从死敌到被迫结盟...',
            },
            {
                vodId: '202167',
                title: '法医秦明之蚀骨时差',
                cover: 'https://hhmage.com/cover/109bcf8ddba64394d5a909f2482b1cc6.jpg',
                badge: '推理',
                badgeColor: 'bg-cyan-500 text-white',
                episode: '第14集',
                desc: '两代警察生死接力、跨越三十年追凶真相...',
            },
            {
                vodId: '201299',
                title: '龙婆虎',
                cover: 'https://hhmage.com/cover/ec5935f52ea59fbad054b523ccdf9c72.jpg',
                badge: '剧情',
                badgeColor: 'bg-amber-500 text-black font-black',
                episode: '第17集',
                desc: '龙婆和伙伴们意外卷入一起钻石失窃案与离奇凶杀...',
            },
            {
                vodId: '202324',
                title: '未来全明星第八季',
                cover: 'https://hhmage.com/cover/6a5a7b235c415f31e1223940c2c456f2.jpg',
                badge: '戏剧',
                badgeColor: 'bg-indigo-500 text-white',
                episode: '第8期',
                desc: '橄榄球少年重返赛场，追逐职业梦想与青春羁绊...',
            },
            {
                vodId: '201828',
                title: '两颗心',
                cover: 'https://hhmage.com/cover/5f2484cfb8073580c9136f7a8894734e.jpg',
                badge: '戏剧',
                badgeColor: 'bg-amber-500 text-black font-black',
                episode: '第5集',
                desc: '泰剧经典虐恋，双生兄弟与豪门千金的情感纠葛...',
            },
            {
                vodId: '202359',
                title: '醒来',
                cover: 'https://hhmage.com/cover/ba9bce0df1d163c5df894ffe0c8d1bca.jpg',
                badge: '豆瓣热榜',
                badgeColor: 'bg-amber-500 text-black font-black',
                episode: '第4集',
                desc: '1941年冬，照相师陆开来在旧父遗物中发现绝密潜伏名单...',
            },
            {
                vodId: '201407',
                title: '35岁了，还谈什么恋爱',
                cover: 'https://hhmage.com/cover/2782341594899d3c425b4fbfbb532d32.jpg',
                badge: '剧情',
                badgeColor: 'bg-rose-500 text-white',
                episode: '第8集',
                desc: '到了这个年纪，已经不会向男人撒娇了，大龄职场女性的现实爱情选择...',
            },
            {
                vodId: '200137',
                title: '想要幸福的政宗君',
                cover: 'https://static.huarenlivewebsite.top/piccdn/2/16/xiang_yao_xing_fu_de_zheng_zong_jun/200137_image.png',
                badge: '剧情',
                badgeColor: 'bg-blue-500 text-white',
                episode: '08集全',
                desc: '本剧改编自同名漫画，政宗君寻找真爱与自我救赎的暖心故事...',
            },
        ],
        rankings: [
            { rank: 1, title: '逐玉', status: '40集全' },
            { rank: 2, title: '莫离', status: '非璃墨属跑男默契大挑战集全' },
            { rank: 3, title: '九门', status: '彩蛋04集全' },
            { rank: 4, title: '百花杀', status: '番外集全' },
            { rank: 5, title: '正义女神粤语', status: '25集全' },
            { rank: 6, title: '御廷谣', status: '32' },
            { rank: 7, title: '铁拳教育', status: '10集全' },
            { rank: 8, title: '骄阳似我', status: '36集全' },
            { rank: 9, title: '这一秒过火', status: '33集全' },
            { rank: 10, title: '新闻女王2粤语', status: '25' },
        ],
    },
    {
        id: 'varieties',
        title: '最新综艺',
        rankTitle: '最新综艺榜单',
        type: 'tv',
        tags: ['大陆综艺', '日韩综艺', '港台综艺', '欧美综艺'],
        cards: [
            {
                vodId: '201039',
                title: '心动双重奏',
                cover: 'https://hhmage.com/cover/ad595f77f52724b0e11d07b27f2a5176.jpg',
                badge: '真人秀',
                badgeColor: 'bg-emerald-500 text-white',
                episode: '更新中',
                desc: '《心动双重奏》首档代际交友真人秀，8位单身青年携长辈开启寻爱之旅...',
            },
            {
                vodId: '201813',
                title: '宠宠冲冲冲',
                cover: 'https://hhmage.com/cover/da4de9ac9388cf3fba1916019468d9e8.jpg',
                badge: '萌宠',
                badgeColor: 'bg-amber-500 text-black font-black',
                episode: '第10期',
                desc: '《宠宠冲冲冲》以“人宠共生”为核心，萌宠达人趣味大闯关...',
            },
            {
                vodId: '431',
                title: '同床异梦2：你是我的命运',
                cover: 'https://hhmage.com/cover/f58d720bf94dad5b8f6fba554975ebc4.jpg',
                badge: '夫妻观察',
                badgeColor: 'bg-blue-500 text-white',
                episode: '每周一更',
                desc: 'SBS王牌综艺，真实记录各对明星夫妻婚后生活酸甜苦辣...',
            },
            {
                vodId: '201037',
                title: '萌宠来啦',
                cover: 'https://hhmage.com/cover/01255d6683efe077d3de805a0b7104d7.jpg',
                badge: '治愈',
                badgeColor: 'bg-emerald-500 text-white',
                episode: '第12期',
                desc: '全国可爱萌宠大集结，治愈所有不开心...',
            },
            {
                vodId: '165988',
                title: '全民星攻略',
                cover: 'https://www.mdzypic.com/upload/vod/20240614-1/4f527a9700ef40cc0d7c3ffb56386eb6.jpg',
                badge: '益智问答',
                badgeColor: 'bg-purple-600 text-white',
                episode: '20260826',
                desc: '曾国城主持的超高能益智解谜对决综艺...',
            },
            {
                vodId: '199645',
                title: '伟大的导游3',
                cover: 'https://hhmage.com/cover/5d72e77238e2a556a48f39e1403faff3.jpg',
                badge: '旅行户外',
                badgeColor: 'bg-blue-500 text-white',
                episode: '第8期',
                desc: '跟着本地向导探寻世界上最隐秘的美景与美食...',
            },
            {
                vodId: '214',
                title: '小姐不熙娣',
                cover: 'https://hhmage.com/cover/d029847ab0189f1a6c570f1d37ccb6cb.jpg',
                badge: '爆笑访谈',
                badgeColor: 'bg-rose-500 text-white',
                episode: '20260825',
                desc: '小S徐熙娣领衔超犀利女性不设限脱口秀...',
            },
            {
                vodId: '95882',
                title: '娱乐百分百',
                cover: 'https://www.mdzypic.com/upload/vod/20240630-1/e047234133f85e6a0e8a8d8af3f77c2a.jpg',
                badge: '狼人杀',
                badgeColor: 'bg-amber-500 text-black font-black',
                episode: '20260824',
                desc: '经典高能桌游狼人杀与偶像搞笑竞技...',
            },
            {
                vodId: '201836',
                title: 'TheOneShot',
                cover: 'https://hhmage.com/cover/5dfde97d835853b9720acc66b472b6ef.jpg',
                badge: '音乐竞技',
                badgeColor: 'bg-cyan-500 text-white',
                episode: '第6期',
                desc: '顶级音乐人一镜到底真唱挑战舞台...',
            },
            {
                vodId: '200645',
                title: '你好湖南',
                cover: 'https://hhmage.com/cover/bb076991c5c7cb0165f9c389560878a5.jpg',
                badge: '文旅体验',
                badgeColor: 'bg-emerald-500 text-white',
                episode: '第5期',
                desc: '打卡湖湘大地秀丽山水与非遗美食文化...',
            },
        ],
        rankings: [
            { rank: 1, title: 'Running Man', status: '20260824' },
            { rank: 2, title: '喜欢你我也是第六季', status: '20260823(第12期陪看(三))' },
            { rank: 3, title: '乘风2026', status: '20260703(加更版)' },
            { rank: 4, title: '半熟恋人第五季', status: '20260816(特别加更)' },
            { rank: 5, title: '哈哈哈哈哈第六季', status: '20260710(加更版)' },
            { rank: 6, title: '天赐的声音第七季', status: '20260721' },
            { rank: 7, title: '地球超新鲜第二季', status: '20260821' },
            { rank: 8, title: '密室大逃脱第八季', status: '20260822' },
            { rank: 9, title: '心动的信号第九季', status: '20260826(超前彩蛋)' },
            { rank: 10, title: '歌手2024', status: '20260726(巅峰之夜)' },
        ],
    },
    {
        id: 'animes',
        title: '最热动漫',
        rankTitle: '最热动漫榜单',
        type: 'tv',
        tags: ['国产动漫', '日韩动漫', '欧美动漫', '港台动漫'],
        cards: [
            {
                vodId: '122',
                title: '凡人修仙传',
                cover: 'https://static.huarenlivewebsite.top/piccdn/4/23/fan_ren_xiu_xian_chuan/122_image.png',
                badge: '热映推荐',
                badgeColor: 'bg-blue-500 text-white',
                episode: '188',
                desc: '普通的山村穷小子韩立修仙传奇，杀伐果断逆天成仙...',
            },
            {
                vodId: '109',
                title: '仙逆',
                cover: 'https://hhmage.com/cover/896b690b9566f53a875e03a3a324c091.jpg',
                badge: '豆瓣热榜',
                badgeColor: 'bg-amber-500 text-black font-black',
                episode: '155',
                desc: '王林踏上修真大道，平定天下动乱，顺为凡逆为仙...',
            },
            {
                vodId: '91978',
                title: '牧神记',
                cover: 'https://hhmage.com/cover/1303f700901b0618800c94479308df8d.jpg',
                badge: '豆瓣热榜',
                badgeColor: 'bg-amber-500 text-black font-black',
                episode: '第32集',
                desc: '大墟残老村走出的少年秦牧，开启浩瀚神话史诗...',
            },
            {
                vodId: '2845',
                title: '沧元图',
                cover: 'https://hhmage.com/cover/af5604111becd28211b2bc271c8c9b57.jpg',
                badge: '玄幻动作',
                badgeColor: 'bg-purple-600 text-white',
                episode: '更新中',
                desc: '孟川觉醒雷霆神力，一人一刀斩尽妖族大妖...',
            },
            {
                vodId: '189909',
                title: '光阴之外',
                cover: 'https://hhmage.com/cover/d852e3658138a2e06e1105e30ebb3d49.jpg',
                badge: '神作',
                badgeColor: 'bg-rose-500 text-white',
                episode: '第28集',
                desc: '神明睁眼，天地残破，许青自泥沼中走出成就不死杀神...',
            },
            {
                vodId: '142',
                title: '完美世界',
                cover: 'https://hhmage.com/cover/c68df1093a93023d0960841f27224080.jpg',
                badge: '修仙',
                badgeColor: 'bg-emerald-500 text-white',
                episode: '更新中',
                desc: '一粒尘可填海，一根草斩尽日月星辰，荒天帝传奇...',
            },
            {
                vodId: '93084',
                title: '斗破苍穹年番',
                cover: 'https://img.jisuimage.com/cover/5988319f8fdeb1b2d254a9a38518f52e.jpg',
                badge: '异火',
                badgeColor: 'bg-amber-500 text-black font-black',
                episode: '更新中',
                desc: '萧炎掌控多种异火威震斗气大陆，三十年河东三十年河西...',
            },
            {
                vodId: '91093',
                title: '吞噬星空',
                cover: 'https://static.huarenlivewebsite.top/imagecdn/4/23/tun_shi_xing_kong/91093_image.png',
                badge: '机甲科幻',
                badgeColor: 'bg-blue-500 text-white',
                episode: '更新中',
                desc: '罗峰突破基因桎梏成为宇宙强者，守护地球家园...',
            },
            {
                vodId: '180956',
                title: '剑来第二季',
                cover: 'https://img.jisuimage.com/cover/46ca629891f3c3bd16b4a4978d31e483.jpg',
                badge: '国风武侠',
                badgeColor: 'bg-cyan-500 text-white',
                episode: '更新中',
                desc: '陈平安背着剑匣踏足江湖，天下神仙皆为过客...',
            },
            {
                vodId: '23',
                title: '遮天',
                cover: 'https://hhmage.com/cover/eabe8e1fe6add3ddf6d65b6df954b376.jpg',
                badge: '史诗',
                badgeColor: 'bg-indigo-500 text-white',
                episode: '更新中',
                desc: '九龙拉棺开启浩瀚星空仙路，叶凡登临宇宙之巅...',
            },
        ],
        rankings: [
            { rank: 1, title: '凡人修仙传', status: '188集全' },
            { rank: 2, title: '仙逆', status: '155集全' },
            { rank: 3, title: '牧神记', status: '更新中' },
            { rank: 4, title: '沧元图', status: '更新中' },
            { rank: 5, title: '光阴之外', status: '更新中' },
            { rank: 6, title: '完美世界', status: '更新中' },
            { rank: 7, title: '斗破苍穹年番', status: '更新中' },
            { rank: 8, title: '吞噬星空', status: '更新中' },
            { rank: 9, title: '剑来第二季', status: '更新中' },
            { rank: 10, title: '遮天', status: '更新中' },
        ],
    },
];
