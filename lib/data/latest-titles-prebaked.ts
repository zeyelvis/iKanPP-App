 ...
 * 由 scripts/sync-latest-titles.mjs 与 scripts/sync-juliang-short-dramas.mjs 定时自动巡检生成
 * 涵盖全站、电影、电视剧、动漫、综艺、纪录片、微短剧 7 大专区真实 24h 最新入库影视
 */
...
export interface LatestPrebakedItem {
  ...
  rate: string;
  year: string;
  type: 'movie' | 'tv' | 'anime' | 'variety' | 'documentary' | 'short' | string;
  channelKey: 'all' | 'movie' | 'tv' | 'anime' | 'variety' | 'documentary' | 'short' | string;
  genres: string[];
  updateBadge: string;
  createdAt: string;
}
